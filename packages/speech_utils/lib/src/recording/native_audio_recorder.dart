...
final class NativeAudioRecorder {
  ...
  /// Starts continuous wake-word driven command capture.
  ///
  /// This owns a single native PCM stream. Wake-word detection, live command
  /// streaming, and finalized command buffering all consume that same stream so
  /// callers do not need to start a second recorder session while listening.
  Future<VoiceActionCaptureSession> startVoiceActionCapture(
    VoiceActionCaptureRequest request,
  ) async {
    _ensureSupportedPlatform();
    _ensureIdle();
    request.validate();

    final commandSplitOptions = _voiceActionCommandSplitOptions(
      audio: request.audio,
      command: request.command,
    );
    final resolvedBackend = resolveSpeechVadBackend(
      options: commandSplitOptions,
      config: request.command.vad,
    );
    ...
    final wakeWordController = StreamController<WakeWordEvent>();
    final commandStreamController = StreamController<VoiceActionCommandStream>();
    final commandController = StreamController<VoiceActionCommand>();
    final stateController = StreamController<VoiceActionCaptureStateSample>();
    final doneCompleter = Completer<VoiceActionCaptureStopResult>();
    final operationQueue = _AsyncSerialOperationQueue();
    final commandByteLimit = _bytesForDuration(
      duration: request.command.maxCommandDuration,
      sampleRateHz: request.audio.sampleRateHz,
      channelCount: request.audio.channelCount,
    );

    _VoiceActionActiveCommand? activeCommand;
    var capturePaused = false;
    var commandIndex = 0;
    var wakeWordCount = 0;
    var commandCount = 0;
    var postWakeDiscardBytesRemaining = 0;

    void emitState(VoiceActionCaptureState state) {
      if (stateController.isClosed) {
        return;
      }
      stateController.add(VoiceActionCaptureStateSample(at: DateTime.now(), state: state));
    }

    void startCommand(WakeWordEvent event) {
      commandIndex++;
      final startedAt = DateTime.now();
      final audioController = StreamController<Uint8List>();
      final commandCompleter = Completer<VoiceActionCommand>();
      activeCommand = _VoiceActionActiveCommand(
        id: commandIndex,
        wakeWord: event.keyword,
        startedAt: startedAt,
        audioController: audioController,
        completed: commandCompleter,
        splitter: Pcm16StreamPauseSplitter(
          options: commandSplitOptions,
          vadBackend: resolvedBackend.backend,
        ),
      );
      commandStreamController.add(
        VoiceActionCommandStream(
          id: commandIndex,
          wakeWord: event.keyword,
          startedAt: startedAt,
          pcm16leStream: audioController.stream,
          completed: commandCompleter.future,
        ),
      );
      emitState(VoiceActionCaptureState.capturingCommand);
    }

    Future<void> completeActiveCommand({bool flush = false}) async {
      final command = activeCommand;
      if (command == null || command.completing) {
        return;
      }
      command.completing = true;
      final snippets = flush ? command.splitter.flush() : command.pendingSnippets;
      command.pendingSnippets = const <Pcm16Snippet>[];
      final bytes = snippets.isEmpty ? command.rawBytes.takeBytes() : snippets.first.asBytesView();
      final completed = VoiceActionCommand(
        id: command.id,
        wakeWord: command.wakeWord,
        startedAt: command.startedAt,
        completedAt: DateTime.now(),
        pcm16leBytes: _copyBytes(bytes),
        sampleRateHz: request.audio.sampleRateHz,
        channelCount: request.audio.channelCount,
      );
      commandCount++;
      commandController.add(completed);
      if (!command.completed.isCompleted) {
        command.completed.complete(completed);
      }
      unawaited(command.audioController.close());
      activeCommand = null;
      request.detector.reset();
      emitState(VoiceActionCaptureState.listening);
    }

    Future<void> failActiveCommand(Object error, StackTrace stackTrace) async {
      final command = activeCommand;
      if (command == null) {
        return;
      }
      if (!command.completed.isCompleted) {
        command.completed.completeError(error, stackTrace);
      }
      command.audioController.addError(error, stackTrace);
      unawaited(command.audioController.close());
      activeCommand = null;
    }

    Future<void> handleChunk(Uint8List chunk) async {
      if (capturePaused || chunk.isEmpty) {
        return;
      }

      final command = activeCommand;
      if (command == null) {
        final events = request.detector.addChunk(
          chunk,
          sampleRateHz: request.audio.sampleRateHz,
          channelCount: request.audio.channelCount,
          config: request.wakeWords,
        );
        if (events.isEmpty) {
          return;
        }
        ...
        wakeWordCount++;
        wakeWordController.add(event);
        if (request.command.discardWakeWordAudio) {
          postWakeDiscardBytesRemaining = _bytesForDuration(
            duration: request.command.postWakeDelay,
            sampleRateHz: request.audio.sampleRateHz,
            channelCount: request.audio.channelCount,
          );
          startCommand(event);
      ...
      var commandChunk = chunk;
      if (postWakeDiscardBytesRemaining > 0) {
        final skipped = math.min(postWakeDiscardBytesRemaining, commandChunk.lengthInBytes);
        postWakeDiscardBytesRemaining -= skipped;
        if (skipped >= commandChunk.lengthInBytes) {
          return;
        }
        commandChunk = Uint8List.sublistView(commandChunk, skipped);
      }

      final currentCommand = activeCommand;
      if (currentCommand == null || commandChunk.isEmpty) {
        return;
      }

      final copiedChunk = _copyBytes(commandChunk);
      currentCommand.rawBytes.add(copiedChunk);
      currentCommand.audioController.add(copiedChunk);
      final snippets = currentCommand.splitter.addChunk(copiedChunk);
      if (snippets.isNotEmpty) {
        currentCommand.pendingSnippets = snippets;
        await completeActiveCommand();
        return;
      }
      if (currentCommand.rawBytes.length >= commandByteLimit) {
        await completeActiveCommand(flush: true);
      }
    }
    ...
    unawaited(() async {
      try {
        await for (final chunk in pcmStream) {
          await operationQueue.run(() => handleChunk(chunk));
        }
        await operationQueue.run(() async {
          await completeActiveCommand(flush: true);
        });
        if (!doneCompleter.isCompleted) {
          doneCompleter.complete(
            VoiceActionCaptureStopResult(commandCount: commandCount, wakeWordCount: wakeWordCount),
          );
        }
      } on Object catch (error, stackTrace) {
        wakeWordController.addError(error, stackTrace);
        commandStreamController.addError(error, stackTrace);
        commandController.addError(error, stackTrace);
        stateController.addError(error, stackTrace);
        await failActiveCommand(error, stackTrace);
        if (!doneCompleter.isCompleted) {
          doneCompleter.completeError(error, stackTrace);
        }
      } finally {
        resolvedBackend.backend.dispose();
        request.detector.dispose();
        emitState(VoiceActionCaptureState.stopped);
        await wakeWordController.close();
        await commandStreamController.close();
        await commandController.close();
        await stateController.close();
      }
    }());

    return _VoiceActionCaptureSessionImpl(
      wakeWords: wakeWordController.stream,
      commandStreams: commandStreamController.stream,
      ...
      pauseFn: () {
        return operationQueue.run(() async {
          ...
          capturePaused = true;
          await pause();
          emitState(VoiceActionCaptureState.paused);
        });
      },
      resumeFn: () {
        return operationQueue.run(() async {
          ...
          emitState(
            activeCommand == null
                ? VoiceActionCaptureState.listening
                : VoiceActionCaptureState.capturingCommand,
          );
        });
      },
      stopFn: () async {
...
int _bytesForDuration({
  ...
  return (bytesPerSecond * duration.inMicroseconds / Duration.microsecondsPerSecond).round();
}

Uint8List _copyBytes(Uint8List bytes) {
  final copy = Uint8List(bytes.lengthInBytes);
  copy.setRange(0, copy.lengthInBytes, bytes);
  return copy;
}

PauseSplitOptions _voiceActionCommandSplitOptions({
  required AudioRecorderConfig audio,
  required WakeCommandCaptureConfig command,
}) {
  return PauseSplitOptions(
    sampleRateHz: audio.sampleRateHz,
    channelCount: audio.channelCount,
    frameDuration: command.frameDuration,
    minSpeechDuration: command.minCommandDuration,
    minSilenceDuration: command.endSilenceDuration,
    preSpeechPadding: command.preSpeechPadding,
    postSpeechPadding: command.trailingPadding,
  );
}
...
final class _VoiceActionActiveCommand {
  _VoiceActionActiveCommand({
    required this.id,
  ...
  final Completer<VoiceActionCommand> completed;
  final Pcm16StreamPauseSplitter splitter;
  final BytesBuilder rawBytes = BytesBuilder(copy: false);
  List<Pcm16Snippet> pendingSnippets = const <Pcm16Snippet>[];
  bool completing = false;
}

final class _VoiceActionCaptureSessionImpl implements VoiceActionCaptureSession {
  _VoiceActionCaptureSessionImpl({
    required this.wakeWords,
    required this.commandStreams,
  ...
  final Future<VoiceActionCaptureStopResult> Function() _stopFn;
  final Future<void> Function() _cancelFn;
  Future<VoiceActionCaptureStopResult>? _stopFuture;
  Future<void>? _cancelFuture;

  @override
  final Stream<WakeWordEvent> wakeWords;

  @override
  final Stream<VoiceActionCommandStream> commandStreams;

  @override
  final Stream<VoiceActionCommand> commands;

  @override
  final Stream<VoiceActionCaptureStateSample> states;
