...
const _requiredImportLibPrefixes = <String>[
  'avcodec',
  'avformat',
  'avutil',
  'swresample',
];
const _requiredRuntimeDllPrefixes = <String>[
  'avcodec',
  'avformat',
  'avutil',
  'swresample',
...
Future<WindowsFfmpegSdk> loadWindowsFfmpegSdk(
  ...
  final rootDir = Directory.fromUri(
    input.packageRoot.resolve('third_party/ffmpeg/windows'),
  );
  ...
  final resolvedImportLibs = _resolveRequiredImportLibs(
    libDir: libDir,
    binDir: binDir,
  );
  if (output != null) {
...
List<File> _resolveBundledRuntimeDlls(Directory binDir) {
  ...
  for (final prefix in [
    ..._requiredRuntimeDllPrefixes,
    ..._optionalRuntimeDllPrefixes,
  ]) {
...
File? _pickLatestRuntimeDllForPrefix(
  Map<String, File> allDllsByLowerName,
  String prefix,
) {
...
void _validateRequiredRuntimeDllPrefixes(
  List<File> runtimeDlls,
  ...
  for (final requiredPrefix in _requiredRuntimeDllPrefixes) {
    final hasPrefix = runtimeDllNames.any(
      (name) => name.startsWith(requiredPrefix),
    );
    if (!hasPrefix) {
      throw StateError(
...
void _validateTransitiveRuntimeDllCompatibility(
  List<File> runtimeDlls,
  String ffmpegBinPath,
) {
  ...
  void validateDependency({
    ...
    if (dependency == null) {
      issues.add(
        'Missing $dependencyFileNameLower required by ${p.basename(importer.path)}.',
      );
      return;
    }
