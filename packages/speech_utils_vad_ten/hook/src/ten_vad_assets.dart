...
const _tenVadAssetName = 'src/generated/ten_vad_bindings.dart';
const _tenVadLinuxRuntimeAssetNamePrefix =
    'src/generated/vad/linux_ten_vad_runtime';
const _tenVadLibraryBaseName = 'speech_utils_ten_vad';
const _tenVadIosSimulatorStubSource =
    'native/ios/speech_utils_ten_vad_simulator_stub.c';

Future<void> bundleTenVadAsset(
  BuildInput input,
  ...
  final bundledFileName = input.config.code.targetOS.dylibFileName(
    _tenVadLibraryBaseName,
  );
  final bundledLibrary = input.outputDirectory.resolve(
    'speech_utils_vad_ten/$bundledFileName',
  );

  final bundledFile = File.fromUri(bundledLibrary);
  await _copyLibraryForTarget(
    input: input,
    sourceFile: sourceFile,
    bundledFile: bundledFile,
  );
  _bundleTenVadRuntimeLibraries(
    input: input,
    output: output,
...
Uri? _tenVadSourceLibraryUri(BuildInput input) {
  ...
  if (os == OS.windows) {
    return switch (arch) {
      Architecture.x64 => root.resolve(
        'third_party/ten_vad/lib/windows/x64/ten_vad.dll',
      ),
      _ => null,
    };
  }
  if (os == OS.linux) {
    return switch (arch) {
      Architecture.x64 => root.resolve(
        'third_party/ten_vad/lib/linux/x64/libten_vad.so',
      ),
      _ => null,
    };
  }
  if (os == OS.android) {
    return switch (arch) {
      Architecture.arm64 => root.resolve(
        'third_party/ten_vad/lib/android/arm64-v8a/libten_vad.so',
      ),
      Architecture.arm => root.resolve(
        'third_party/ten_vad/lib/android/armeabi-v7a/libten_vad.so',
      ),
      _ => null,
    };
  }
  if (os == OS.iOS) {
    ...
    return switch (arch) {
      Architecture.arm64 => root.resolve(
        'third_party/ten_vad/lib/ios/ten_vad.framework/ten_vad',
      ),
      _ => null,
    };
  }
...
List<File> _tenVadRuntimeLibraries(
  BuildInput input,
  BuildOutputBuilder output,
) {
