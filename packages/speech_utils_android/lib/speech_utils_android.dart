...
import 'src/generated/android_jni_bindings.dart' as android_jni;
import 'src/android_aac_encoder.dart';

/// Android AudioRecord/MediaRecorder implementation for `speech_utils`.
final class SpeechUtilsAndroid extends SpeechUtilsPlatform {
  const SpeechUtilsAndroid()
    : super(
  ...
  static void registerWith() {
    SpeechUtilsPlatform.instance = const SpeechUtilsAndroid();
  }

  @override
  NativeAacEncoderBackend get aacEncoder => const AndroidAacEncoderBackend();
...
int _resolveEncodingBitrateBps(AudioEncodingConfig encoding) {
  final configured = encoding.bitrateBps;
  return configured != null && configured > 0 ? configured : 64000;
}
...
final class _AndroidJniAudioRecordBackend {
  ...
  Future<bool> requestPermission() async {
    ...
    final activity = rawActivity.as(
      android_jni.Activity.type,
  ...
  bool _hasRecordAudioPermission() {
    ...
    try {
      ...
      typedContext = rawContext.as(
        android_jni.Context.type,
        releaseOriginal: true,
      );
      rawContext = null;
  ...
  Future<bool> _requestRecordAudioPermission(
    ...
    try {
      ...
      final initialShouldShowRationale =
          _safeShouldShowRequestPermissionRationale(
            activity,
            permission,
            fallback: false,
          );
      final initialHasWindowFocus = _safeHasWindowFocus(
        activity,
        fallback: true,
      );
  ...
  Future<bool> _waitForPermissionRequestResolution({
    ...
    while (DateTime.now().difference(startedAt) <
        _permissionResolutionTimeout) {
      if (_hasRecordAudioPermission()) {
        return true;
      }

      final hasWindowFocus = _safeHasWindowFocus(
        activity,
        fallback: initialHasWindowFocus,
      );
      ...
      if (!sawDialogFocusLoss &&
          DateTime.now().difference(startedAt) >=
              _permissionDialogDetectionTimeout) {
        return _hasRecordAudioPermission();
      }
  ...
  int _resolveSourcePolicyCode(AudioRecorderConfig config) {
    return switch (config.processing.preset) {
      ...
      AudioCapturePreset.voiceIsolation => _sourcePolicyVoice,
    };
  }
...
final class _AndroidMediaRecorderWorkerBridge
    implements _AndroidRecorderWorkerBridge {
  _AndroidMediaRecorderWorkerBridge._(this._instance);

  static const _className =
      'org/hippolabs/speech_utils/SpeechUtilsMediaRecorderWorker';
  ...
  static final _isRecordingMethod = _class.instanceMethodId(
    'isRecording',
    '()Z',
  );
  static final _currentDbfsMethod = _class.instanceMethodId(
    'getCurrentDbfs',
    '()D',
  );
  static final _maxDbfsMethod = _class.instanceMethodId('getMaxDbfs', '()D');
  static final _activeSampleRateMethod = _class.instanceMethodId(
    'getActiveSampleRateHz',
    '()I',
  );
  static final _activeChannelCountMethod = _class.instanceMethodId(
...
final class _AndroidAudioRecordWorkerBridge
  ...
  static const _className =
      'org/hippolabs/speech_utils/SpeechUtilsAudioRecordWorker';
  static final JClass _class = JClass.forName(_className);
  static final _constructor = _class.constructorId('()V');
  static final _startFileMethod = _class.instanceMethodId(
    'startFile',
  ...
  static final _startStreamMethod = _class.instanceMethodId(
    'startStream',
    '(IIII)V',
  );
  static final _readStreamMethod = _class.instanceMethodId(
    'readStream',
    '(I)[B',
  );
  static final _stopMethod = _class.instanceMethodId('stop', '()V');
  ...
  Uint8List readStream({required int maxBytes}) {
    final array = _readStreamMethod.call(_instance, JByteArray.type, [
      maxBytes,
    ]);
    try {
