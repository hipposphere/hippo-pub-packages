...
class _FileRecordingPageState extends State<FileRecordingPage> {
  ...
  static const Duration _recordingTickerInterval = Duration(milliseconds: 50);
  static const Duration _amplitudePollInterval = Duration(milliseconds: 20);
    ...
    try {
      ...
      if (logResult) {
        _appendLog(
          successMessage ??
              (nextEnabled
                  ? 'Continuous capture enabled for "$_selectedInputLabel" ${_continousCaptureDurationPreset.warmBehaviorLabel}. File recordings will reuse the warm route.'
  ...
  Future<void> _startRecording() async {
    ...
    _recordingStopwatch
      ...
      ..start();
    _recordingTicker?.cancel();
    _recordingTicker = Timer.periodic(_recordingTickerInterval, (_) {
