...
extern "C" __declspec(dllexport) int32_t
speech_utils_windows_audio_recorder_start_file(
    const speech_utils::recorder::RecorderStartConfig* start_config,
    char* error_utf8,
    uint32_t error_utf8_capacity) {
  return speech_utils::windows_recorder::StartFile(start_config, error_utf8, error_utf8_capacity);
}

extern "C" __declspec(dllexport) int32_t
speech_utils_windows_audio_recorder_start_stream(
    const speech_utils::recorder::RecorderStartConfig* start_config,
    char* error_utf8,
    uint32_t error_utf8_capacity) {
  return speech_utils::windows_recorder::StartStream(start_config, error_utf8,
                                                     error_utf8_capacity);
}
...
extern "C" __declspec(dllexport) int32_t
speech_utils_windows_audio_recorder_reset(char* error_utf8, uint32_t error_utf8_capacity) {
  return speech_utils::windows_recorder::Reset(error_utf8, error_utf8_capacity);
}
