...
WindowsWebRtcApmSdk requireVendoredWindowsWebRtcApmSdk(
  ...
  final rootDir = Directory.fromUri(
    input.packageRoot.resolve('third_party/webrtc_apm/windows'),
  );
  if (!rootDir.existsSync()) {
    throw StateError(
      'Missing vendored WebRTC APM SDK directory at ${rootDir.path}.',
    );
  }
  ...
  if (output != null) {
    output.dependencies.add(
      File(p.join(includeDir.path, _requiredHeaderPath)).absolute.uri,
    );
  }

  final importLib = _findImportLibByPreferredPrefixes(
    libDir: libDir,
    binDir: binDir,
  );
  if (importLib == null) {
  ...
  final libraries = [
    libraryName,
    ..._collectAdditionalLibraryNames(libDir, binDir, libraryName),
  ];
...
Directory? _resolveIncludeDir(Directory includeRootDir) {
  ...
  final prefixedDir = Directory(
    p.join(includeRootDir.path, 'webrtc-audio-processing-1'),
  );
...
List<File> _resolveRuntimeDlls(Directory binDir, List<String> libraryNames) {
  ...
  final lowerLibraryNames = libraryNames
      ...
      binDir
          ...
          .where((file) {
            ...
            for (final libraryName in lowerLibraryNames) {
              if (fileName == '$libraryName.dll' ||
                  fileName.startsWith('$libraryName-')) {
                return true;
              }
            }
            return false;
...
List<String> _collectAdditionalLibraryNames(
  ...
  names.removeWhere(
    (name) => name.toLowerCase() == primaryLibraryName.toLowerCase(),
  );
