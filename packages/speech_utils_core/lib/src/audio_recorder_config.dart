...
final class AudioProcessingConfig {
  ...
  /// Apple capture is session-mode driven, so this always returns `false` for
  /// [AudioProcessingPlatform.ios] and [AudioProcessingPlatform.macos].
  bool resolveAutomaticGainControlForPlatform(
    AudioProcessingPlatform platform,
  ) {
    if (platform == AudioProcessingPlatform.ios ||
        platform == AudioProcessingPlatform.macos) {
      return false;
...
final class IosAudioRecorderConfig {
  ...
  void validate() {
    ...
    if (preferredIoBufferDuration != null &&
        preferredIoBufferDuration! <= Duration.zero) {
      throw ArgumentError.value(
...
final class MacosAudioRecorderConfig {
  ...
  void validate() {
    if (processingQueueDuration != null &&
        processingQueueDuration! <= Duration.zero) {
      throw ArgumentError.value(
...
final class WindowsAudioRecorderConfig {
  ...
  void validate() {
    if (targetBufferDuration != null &&
        targetBufferDuration! <= Duration.zero) {
      throw ArgumentError.value(
