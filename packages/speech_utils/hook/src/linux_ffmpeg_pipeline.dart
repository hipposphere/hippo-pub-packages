...
const _requiredSharedLibraryPrefixes = <String>['avcodec', 'avformat', 'avutil', 'swresample'];
const _optionalSharedLibraryPrefixes = <String>['avdevice', 'avfilter', 'swscale'];
...
Future<LinuxFfmpegSdk> loadLinuxFfmpegSdk(BuildInput input, {BuildOutputBuilder? output}) async {
  final rootDir = Directory.fromUri(input.packageRoot.resolve('third_party/ffmpeg/linux'));
...
Map<String, File> _resolveRequiredLinkerLibraries(Directory libDir) {
  ...
  if (missingPrefixes.isNotEmpty) {
    final missingNames = missingPrefixes.map((prefix) => 'lib$prefix.so').join(', ');
    throw StateError(
...
List<File> _resolveBundledRuntimeLibraries(Directory libDir) {
  ...
  for (final prefix in [..._requiredSharedLibraryPrefixes, ..._optionalSharedLibraryPrefixes]) {
    for (final library in _runtimeLibrariesForPrefix(allSharedLibrariesByName, prefix)) {
...
List<File> _runtimeLibrariesForPrefix(Map<String, File> allLibrariesByName, String prefix) {
  final baseName = 'lib$prefix.so';
  final matches = allLibrariesByName.entries
      .where((entry) => entry.key == baseName || entry.key.startsWith('$baseName.'))
      .map((entry) => entry.value)
      .toList(growable: false);

  matches.sort((left, right) => p.basename(left.path).compareTo(p.basename(right.path)));
  return matches;
}

void _validateRequiredRuntimeLibraryPrefixes(List<File> runtimeLibraries, String ffmpegLibPath) {
