...
final class MacosAacEncoderBackend implements NativeAacEncoderBackend {
  const MacosAacEncoderBackend();

  static final _worker = NativeWorkerExecutor(
    entrypoint: _macosAacWorkerMain,
    debugName: 'speech_utils macOS AAC',
  );
...
final class MacosAudioMetadataBackend implements NativeAudioMetadataBackend {
  const MacosAudioMetadataBackend();

  static final _worker = NativeWorkerExecutor(
    entrypoint: _macosMetadataWorkerMain,
    debugName: 'speech_utils macOS metadata',
  );
...
void _macosMetadataWorkerMain(SendPort replyPort) {
  runNativeWorker(
    replyPort,
    (raw) => runMetadataRead(
      metadata_bindings.speech_utils_macos_read_audio_metadata,
      inputPath: raw! as String,
    ),
  );
}

void _macosAacWorkerMain(SendPort replyPort) {
...
void _encode({
  ...
  try {
    final code = bindings.speech_utils_macos_encode_audio_file_to_aac(
      input,
      output,
      bitrateBps,
      error,
