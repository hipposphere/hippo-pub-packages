...
import 'generated/linux_audio_encoder_bindings.dart' as bindings;

const _errorBufferBytes = 4096;

final class LinuxAacEncoderBackend implements NativeAacEncoderBackend {
  const LinuxAacEncoderBackend();

  static final _worker = NativeWorkerExecutor(
    entrypoint: _linuxAacWorkerMain,
    debugName: 'speech_utils Linux AAC',
  );
...
final class LinuxAudioMetadataBackend implements NativeAudioMetadataBackend {
  const LinuxAudioMetadataBackend();

  static final _worker = NativeWorkerExecutor(
    entrypoint: _linuxMetadataWorkerMain,
    debugName: 'speech_utils Linux metadata',
  );

  @override
  bool isAvailable() => runMetadataHealthcheck(
    bindings.speech_utils_linux_audio_metadata_healthcheck,
  );
...
void _linuxMetadataWorkerMain(SendPort replyPort) {
  runNativeWorker(
    replyPort,
    (raw) => runMetadataRead(
      bindings.speech_utils_linux_read_audio_metadata,
      inputPath: raw! as String,
    ),
  );
}

void _linuxAacWorkerMain(SendPort replyPort) {
...
bool _healthcheck() {
  ...
  try {
    try {
      return bindings.speech_utils_linux_aac_encoder_healthcheck(
            error,
            _errorBufferBytes,
          ) ==
          0;
...
void _encode({
  ...
  final error = calloc<ffi.Char>(_errorBufferBytes);
  try {
    final code = bindings.speech_utils_linux_encode_audio_file_to_aac(
      input,
      output,
      bitrateBps,
      error,
